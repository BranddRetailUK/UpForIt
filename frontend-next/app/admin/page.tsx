...
import AdminTierToggle from "../../components/AdminTierToggle";
...
type Tier = { id: string; name: string; price_minor: number; is_active: boolean; paid_quantity: string; reserved_quantity: string };
type Order = { id: string; order_number: string; status: string; total_minor: number; created_at: Date; paid_at: Date | null; confirmation_email_sent_at: Date | null; email: string; display_name: string; event_title: string; ticket_count: string };
...
export default async function AdminPage() {
  ...
  const [metricResult, tiers, orders, emailJobs, webhooks] = await Promise.all([
    ...
    getPool().query<Tier>(
      `SELECT tt.id, tt.name, tt.price_minor, tt.is_active,
  ...
  return (
    <div className="admin-shell section-wrap">
      ...
      <section className="admin-panel">
        <h2>Ticket tiers</h2>
        <p>Only active tiers are available at checkout. Prices are fixed to the confirmed values.</p>
        <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Tier</th><th>Price</th><th>Paid</th><th>Reserved</th><th>Sales</th></tr></thead><tbody>
          {tiers.rows.map((tier) => <tr key={tier.id}><td>{tier.name}</td><td>£{(tier.price_minor / 100).toFixed(2)}</td><td>{tier.paid_quantity}</td><td>{tier.reserved_quantity}</td><td><AdminTierToggle id={tier.id} initialActive={tier.is_active} /></td></tr>)}
        </tbody></table></div>
      </section>
      ...
      <section className="admin-panel">
        ...
        <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Order</th><th>Buyer</th><th>Status</th><th>Tickets</th><th>Total</th><th>Email</th><th>Created</th><th /></tr></thead><tbody>
          {orders.rows.map((order) => <tr key={order.id}>
            <td><Link href={`/account/orders/${order.id}`}>{order.order_number}</Link></td><td>{order.display_name}<small>{order.email}</small></td>
