...
describe("ticket tier progression", () => {
  ...
  it("activates General Release when 50 Early Bird tickets are paid", () => {
    expect(chooseActiveTicketTier(tiers(50, 0))).toBe("tier-1");
  });

  it("keeps General Release active through its first 99 paid tickets", () => {
    expect(chooseActiveTicketTier(tiers(50, 99, "tier-1"))).toBe("tier-1");
  });

  it("activates unlimited On The Door after 100 General Release tickets", () => {
