import type { SOCIAL_LINKS } from "../lib/site";
...
export default function SocialBrandIcon({
  ...
  const gradientId = `${idPrefix}-instagram-brand-gradient`;
  ...
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
      ...
      <rect x="3" y="3" width="18" height="18" rx="5" fill="none" stroke={`url(#${gradientId})`} strokeWidth="2.6" />
      <circle cx="12" cy="12" r="4.2" fill="none" stroke={`url(#${gradientId})`} strokeWidth="2.6" />
      <circle cx="17.55" cy="6.55" r="1.35" fill={`url(#${gradientId})`} />
    </svg>
  );
}
