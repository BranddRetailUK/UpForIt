...
/admin=staff/admin testing control centre for metrics, automatic tier status, ticket purchases excluding expired checkouts, emails, Stripe events, ticket resend, active-campaign Meta analytics with combined/per-campaign tabs, scanner access at the bottom, and an admin-only Testing purchase simulator
